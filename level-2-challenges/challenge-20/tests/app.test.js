...
describe('The challenge', () => {
  ...
  it('should show not connected message if launch is attempted without connection', () => {
    ...
    expect(logItems[0].textContent).toBe(
      'Connection to Major Tom is required.'
    );
  });
  ...
  it('should connect to Major Tom successfully', async () => {
    ...
    const logItems = document.querySelectorAll('#log-list li');
    expect(logItems[0].textContent).toBe(
      'Attempting to connect to Major Tom...'
    );
    expect(logItems[1].textContent).toBe(
      'Connection established with Major Tom.'
    );
  });
  ...
  it('should initiate launch sequence and complete countdown', () => {
    ...
    for (let i = 3; i > 0; i--) {
      vi.advanceTimersByTime(1000);
      expect(document.querySelector('#log-list').innerHTML).toContain(
        `T-minus ${i}`
      );
    }
    ...
    vi.advanceTimersByTime(1000);
    expect(document.querySelector('#log-list').innerHTML).toContain(
      'Launch Successful! Major Tom is in orbit.'
    );
    vi.useRealTimers();
  });
  ...
  it('should abort mission during countdown', () => {
    ...
    const logItems = document.querySelectorAll('#log-list li');
    expect(logItems[logItems.length - 1].textContent).toBe(
      'Mission aborted! Major Tom is safe.'
    );
    vi.useRealTimers();
  });
